...
4. **Subscription and billing.** $4.99/month, charged by Stripe. Automatic renewal until you cancel. No refunds except as noted on the pricing page FAQ.
...
| `test/FAQ.test.tsx` | Clicking/pressing a `<summary>` toggles the `<details>` `open` attribute (native accordion behavior) |
