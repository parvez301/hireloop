...
export function OfferForm({ jobId, onCreated }: OfferFormProps) {
  ...
  return (
    <form onSubmit={(e) => void submit(e)} className="mt-3 max-w-md space-y-2">
      <div>
        <label className="text-xs text-ink-3" htmlFor="offer-base">
          Base salary (USD)
        </label>
        <input
          id="offer-base"
          type="number"
          min={0}
          className="mt-1 w-full rounded border border-line-2 px-2 py-1 text-sm"
          value={base}
          onChange={(e) => setBase(e.target.value)}
        />
      </div>
      <div>
        <label className="text-xs text-ink-3" htmlFor="offer-eq">
          Equity (optional)
        </label>
        <input
          id="offer-eq"
      ...
      <button
        type="submit"
        disabled={sending}
        className="rounded bg-cobalt px-3 py-1 text-sm text-white disabled:opacity-50"
      >
